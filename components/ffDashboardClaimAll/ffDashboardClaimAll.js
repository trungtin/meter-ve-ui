...
export default function ffClaimAll() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      stores.emitter.removeListener(
        ACTIONS.FIXED_FOREX_ALL_CLAIMED,
        claimReturned
      );
  ...
  const getClaimable = () => {
    ...
    if (
      ...
      BigNumber(rewards.feeDistribution.earned).gt(0)
    ) {
      cl.push({
        type: 'Solidly',
    ...
    if (
      ...
      BigNumber(rewards.veIBFFDistribution.earned).gt(0)
    ) {
      cl.push({
        type: 'Solidly',
  ...
  const onClaim = () => {
    ...
    stores.dispatcher.dispatch({
      type: ACTIONS.FIXED_FOREX_CLAIM_ALL,
      content: { claimable },
    });
  };
